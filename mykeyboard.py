...
def PressKey(hexKeyCode, extend=False):
    extra = ctypes.c_ulong(0)
    ii_ = Input_I()
    if extend :
        ii_.ki = KeyBdInput( 0, hexKeyCode, 0x0008 | 0x0001, 0, ctypes.pointer(extra) )
    else:
        ii_.ki = KeyBdInput( 0, hexKeyCode, 0x0008, 0, ctypes.pointer(extra) )
    x = Input( ctypes.c_ulong(1), ii_ )
    ctypes.windll.user32.SendInput(1, ctypes.pointer(x), ctypes.sizeof(x))

def ReleaseKey(hexKeyCode, extend=False):
    extra = ctypes.c_ulong(0)
    ii_ = Input_I()
    if extend :
        ii_.ki = KeyBdInput( 0, hexKeyCode, 0x0008 | 0x0002 | 0x0001, 0, ctypes.pointer(extra) )
    else :
